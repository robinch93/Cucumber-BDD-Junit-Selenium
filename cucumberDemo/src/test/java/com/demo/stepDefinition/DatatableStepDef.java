...
public class DatatableStepDef extends BaseClass {
    ...
    public void user_enter_computer_details(DataTable details) throws Exception {
        ...
        driver.findElement(By.name(Computer.name)).sendKeys(data.get(0).get(0));
        driver.findElement(By.name(Computer.introducedDate)).sendKeys(data.get(0).get(1));
        Thread.sleep(500);
    ...
    public void user_enter_company(DataTable company) {
        ...
        searchBox.sendKeys(data.get(0).get(0));
    }

    @Then("User clicks the filter button")
    public void user_click_filter_button() throws InterruptedException {
        WebElement filterByName = driver.findElement(By.xpath(Computer.filterByName));
        JavascriptExecutor executor = (JavascriptExecutor) driver;
        executor.executeScript("arguments[0].click();", filterByName);
        Thread.sleep(500);
    }
